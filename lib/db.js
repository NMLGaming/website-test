...
const SCHEMA_SQL = `
  ...
  CREATE TABLE IF NOT EXISTS mcc_keys (
    id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
    key_code TEXT NOT NULL UNIQUE, ip_address TEXT NOT NULL,
    discord_id TEXT NOT NULL, discord_username TEXT NOT NULL DEFAULT '',
    issued_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL, renewals_used INTEGER NOT NULL DEFAULT 0,
    revoked_at TIMESTAMPTZ, last_seen_at TIMESTAMPTZ
  );
  CREATE TABLE IF NOT EXISTS mcc_activity (
    id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
    key_id TEXT REFERENCES mcc_keys(id) ON DELETE SET NULL,
    ip_address TEXT NOT NULL, discord_id TEXT NOT NULL DEFAULT '',
    discord_username TEXT NOT NULL DEFAULT '', event_type TEXT NOT NULL,
    message TEXT NOT NULL DEFAULT '', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  );
  CREATE TABLE IF NOT EXISTS mcc_ip_blocks (
    ip_address TEXT PRIMARY KEY, reason TEXT NOT NULL DEFAULT '',
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  );
`;

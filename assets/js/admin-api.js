...
const AdminAPI = (function () {
  ...
  return {
    ...
    updateAnnouncement(id, d)   { return _put('/api/announcements/' + id, d); },
    deleteAnnouncement(id)      { return _del('/api/announcements/' + id); },

    // ---- Leaderboard ----
    getLeaderboard(server)         { return _get('/api/leaderboard/' + server); },
    addLbEntry(server, d)          { return _post('/api/leaderboard/' + server, d); },
    updateLbEntry(server, id, d)   { return _put('/api/leaderboard/' + server + '/' + id, d); },
    deleteLbEntry(server, id)      { return _del('/api/leaderboard/' + server + '/' + id); },

    // ---- Players ----
    getPlayers()        { return _get('/api/players'); },
    createPlayer(d)     { return _post('/api/players', d); },
    updatePlayer(id, d) { return _put('/api/players/' + id, d); },
    deletePlayer(id)    { return _del('/api/players/' + id); },

    // ---- Settings ----
    getSettings()      { return _get('/api/settings'); },
    updateSettings(d)  { return _put('/api/settings', d); },

...
(function () {
  ...
  function saveAnnouncement() {
    ...
    var url    = editingId ? '/api/announcements?id=' + editingId : '/api/announcements';
    ...
    fetch(url, {
      ...
      .then(function (r) {
        if (!r.ok) return r.json().then(function (d) { throw new Error(d.error || 'Lỗi lưu'); });
        return r.json();
      })
      .then(function () {
        closeModal();
  ...
  function deleteAnnouncement(id) {
    fetch('/api/announcements?id=' + id, { method: 'DELETE', credentials: 'include' })
      .then(function (r) {
        if (!r.ok) return r.json().then(function (d) { throw new Error(d.error || 'Lỗi xoá'); });
  ...
  function togglePin(item) {
    ...
    fetch('/api/announcements?id=' + item.id, {
      method:      'PUT',
      credentials: 'include',
      headers:     { 'Content-Type': 'application/json' },
      body:        JSON.stringify(payload),

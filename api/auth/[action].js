...
const MCC_CHARS       = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
...
function requestIp(req) {
  ...
  return String(forwarded ? forwarded.split(',')[0] : (req.socket && req.socket.remoteAddress || 'unknown')).trim();
}
function makeKey() {
  let out = 'MCC-KEY-';
  for (let i = 0; i < 12; i++) out += MCC_CHARS[crypto.randomInt(MCC_CHARS.length)];
  return out;
}
function mccUser(req) {
  const user = verifyToken(req);
  return user && user._did ? user : null;
}
function keyView(row, reveal) {
  return { id: row.id, key: reveal ? row.key_code : null, expires_at: row.expires_at,
    renewals_used: row.renewals_used, discord_username: row.discord_username, ip_address: row.ip_address };
}
async function handleGetKey(req, res) {
  const user = mccUser(req);
  if (!user) return sendApiError(res, 401, 'Đăng nhập Discord để lấy key');
  if (!DB) return sendApiError(res, 503, 'Tính năng GetKey cần DATABASE_URL');
  const ip = requestIp(req);
  await query('DELETE FROM mcc_keys WHERE expires_at <= NOW()', []);
  const current = await query(
    'SELECT * FROM mcc_keys WHERE (ip_address=$1 OR discord_id=$2) AND revoked_at IS NULL AND expires_at>NOW() ORDER BY expires_at DESC LIMIT 1',
    [ip, user._did]);
  if (req.method === 'GET') return res.status(200).json({ success: true, data: current.rows[0] ? keyView(current.rows[0], true) : null, authenticated: true });
  if (req.method !== 'POST') return sendApiError(res, 405, 'Method not allowed');
  if (current.rows[0]) return res.status(200).json({ success: true, data: keyView(current.rows[0], true), existing: true });
  const code = makeKey();
  const created = await query(
    `INSERT INTO mcc_keys (key_code,ip_address,discord_id,discord_username,expires_at)
     VALUES ($1,$2,$3,$4,NOW()+INTERVAL '24 hours') RETURNING *`,
    [code, ip, user._did, user.username || '']);
  await query('INSERT INTO mcc_activity (key_id,ip_address,discord_id,discord_username,event_type,message) VALUES ($1,$2,$3,$4,$5,$6)',
    [created.rows[0].id, ip, user._did, user.username || '', 'issued', 'GetKey']);
  return res.status(201).json({ success: true, data: keyView(created.rows[0], true), challenge: 3 });
}
async function handleMccValidate(req, res) {
  if (req.method !== 'POST') return sendApiError(res, 405, 'Method not allowed');
  if (!DB) return sendApiError(res, 503, 'Database not configured');
  const body = req.body || {}, ip = requestIp(req), code = String(body.key || '').trim();
  const blocked = await query('SELECT 1 FROM mcc_ip_blocks WHERE ip_address=$1', [ip]);
  if (blocked.rows.length) return res.status(403).json({ success:false, blocked:true, error:'IP đã bị cấm sử dụng MCC' });
  const found = await query('SELECT * FROM mcc_keys WHERE key_code=$1 AND revoked_at IS NULL AND expires_at>NOW()', [code]);
  if (!found.rows[0]) return sendApiError(res, 401, 'Key không hợp lệ hoặc đã hết hạn');
  const row = found.rows[0];
  await query('UPDATE mcc_keys SET last_seen_at=NOW() WHERE id=$1', [row.id]);
  await query('INSERT INTO mcc_activity (key_id,ip_address,discord_id,discord_username,event_type,message) VALUES ($1,$2,$3,$4,$5,$6)',
    [row.id, ip, body.discord_id || '', body.discord_username || '', 'heartbeat', body.message || '']);
  return res.status(200).json({ success:true, valid:true, expires_at:row.expires_at });
}
async function handleMccAdmin(req, res) {
  if (!requireOwner(req, res)) return;
  if (!DB) return sendApiError(res, 503, 'Database not configured');
  if (req.method === 'GET') {
    const keys = await query(`SELECT k.*, COALESCE(json_agg(a ORDER BY a.created_at DESC) FILTER (WHERE a.id IS NOT NULL),'[]') activities
      FROM mcc_keys k LEFT JOIN mcc_activity a ON a.key_id=k.id GROUP BY k.id ORDER BY k.issued_at DESC`, []);
    const blocks = await query('SELECT * FROM mcc_ip_blocks ORDER BY created_at DESC', []);
    return res.status(200).json({ success:true, keys:keys.rows, blocks:blocks.rows });
  }
  const body=req.body||{}, ip=String(body.ip_address||'');
  if (req.method === 'DELETE' && body.key_id) {
    await query('UPDATE mcc_keys SET revoked_at=NOW() WHERE id=$1',[body.key_id]);
    return res.status(200).json({success:true});
  }
  if (req.method === 'POST' && ip) {
    await query('INSERT INTO mcc_ip_blocks(ip_address,reason) VALUES($1,$2) ON CONFLICT(ip_address) DO UPDATE SET reason=$2',[ip,String(body.reason||'Admin block')]);
    await query('UPDATE mcc_keys SET revoked_at=NOW() WHERE ip_address=$1',[ip]);
    return res.status(200).json({success:true});
  }
  if (req.method === 'PUT' && ip) {
    await query('DELETE FROM mcc_ip_blocks WHERE ip_address=$1',[ip]);
    return res.status(200).json({success:true});
  }
  return sendApiError(res,400,'Thiếu dữ liệu');
}
...
async function handleData(req, res) {
  try {
    ...
    if (resource === 'getkey') return await handleGetKey(req, res);
    if (resource === 'mcc-validate') return await handleMccValidate(req, res);
    if (resource === 'mcc-admin') return await handleMccAdmin(req, res);
